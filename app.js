...
// Guard at the top keeps everything frozen on non-playing screens.
...
// Two quadratic Bezier tentacles, one on each side.
// The control point Y alternates between 0.35 and 0.45 of the
// bounding box height (driven by enemy.frame) to fake a swimming
// animation — higher control point = more curved stroke.
...
// Each eye is two concentric circles: radius 4 (white) and radius 2 (purple).
ctx.fillStyle = "#fff";
...
// ============================================================
// DRAWING: BULLETS
// Explosive bullets render as a glowing orb. Regular bullets
// are simple rectangles for clarity at high speed.
//
// The -bullet.h / 2 offset on fillRect centers the rectangle
// vertically on the bullet’s y coordinate instead of anchoring
// at the top-left corner, keeping the visual aligned with the
// collision box.
// ============================================================
...
//
// SINE WAVE ANIMATION: Math.sin(drop.pulse) * 0.3 + 0.7
// sin() oscillates between -1 and +1. Scaling by 0.3 tightens
// the range to [-0.3, 0.3], then shifting by +0.7 maps it to
// [0.4, 1.0] — a gentle breathing glow that never reaches zero.
//
// SPINNING STAR: ctx.translate + ctx.rotate positions and tilts the
// canvas coordinate system before drawing, so the star appears to
// spin in place. spin increases each frame in moveDrops so the
// rotation angle grows over time.
// ============================================================
...
function drawDrop(drop) {
  ...
  // spikeIndex * π / spikeCount steps through the circle in equal increments.
...
ctx.shadowBlur = 0;
  ...
  //   2. quadraticCurveTo — bends around the corner using the actual
  //      corner point as the control point and the curve end as the
  //      destination. This produces a smooth arc of radius r.
  //
...
drawPowerupHUD();
  ...
  // ============================================================
  // GAME LOOP
  // requestAnimationFrame calls loop as fast as the display allows
  // (typically 60 times per second). update() advances all game state,
