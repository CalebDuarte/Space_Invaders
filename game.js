...
// Match the canvas pixel dimensions to the browser window so nothing gets stretched or clipped.
...
// Stars scroll downward. When one exits the bottom it wraps to the top at a new random X so the field never empties out.
function drawStars() {
...
// The best score survives page refreshes. We read it once on load and write it back whenever the player breaks their record.
...
// Resets every state then kicks off wave 1.
...
//how to play overlay
// Draws a semi-transparent instruction panel on the title screen so players know the controls before starting.
...
function spawnParticles(x, y, color, amount = 12) {
  for (let i = 0; i < amount; i++) {
    ...
    // cos(angle) gives the X component and sin(angle) gives the Y component of a unit vector pointing in that direction. Multiplying by speed scales the vector so faster particles travel further each frame.
...
function handleShooting() {
  ...
  if ((keys[" "] || keys['z']) && shootCooldown <= 0) {
    // Center the bullet on the ship. The -4 offset starts it just above the ship's nose rather than at its center.
...
function moveDrops() {
  for (let dropIndex = drops.length - 1; dropIndex >= 0; dropIndex--) {
    ...
    // pulse used in drawDrop for the glow ring animation.
...
function moveEnemies() {
  if (!bossPresent) {
    ...
    // If any enemy is touching either side wall, reverse direction and drop the whole formation down by 16px (classic Space Invaders step-down).
...
// explode powerup
// When an explosive bullet hits, everything within radius pixels takes 1 damage (down from 2). 
// Radius shrunk from 70 to 45px.
...
// The ship is made of layered canvas patha
//math for the ship/enemy: ctx.moveTo / lineTo trace a series of (x, y) points.
...
function drawPlayer() {
  ...
  // that completes one full cycle about every 628ms (2pi × 100ms).
...
//draw enemy
// Each enemy type has its own look so the player can read the battlefield at a glance and prioritise targets.
// math for the arc(): ctx.arc(cx, cy, radius, startAngle, endAngle)
// Angles are measured in radians from the 3 o’clock position,
// going clockwise. 0 → Math.PI * 2 draws a complete circle.
// One full revolution = 2pi radians ≈ 6.28 radians.
...
// Bar width = enemy.w * (hp / maxHp), so it shrinks with health.
...
// wave animation: Math.sin(drop.pulse) * 0.3 + 0.7 sin() oscillates between -1 and +1. Scaling by 0.3 tightens
...
function drawDrop(drop) {
  ...
  if (drop.type === "doubleShot") {
    ...
    // Dividing the full circle (2pi radians) into spikeCount * 2 equal
